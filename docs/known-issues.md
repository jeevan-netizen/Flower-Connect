...
| 017 | Test      | Two separate issues: (a) Testcontainers cannot connect to Docker Desktop 29.8.0 without `~/.docker-java.properties` containing `api.version=1.44` — both `EnvironmentAndSystemPropertyClientProviderStrategy` and `NpipeSocketClientProviderStrategy` fail with `BadRequestException (Status 400)`; (b) `@Container` on `IntegrationTestBase.MYSQL` caused Testcontainers to create and destroy a new MySQL container per test class (per-class lifecycle), with four containers started (one per test class); `AuthApiIntegrationTest` and `RefreshTokenRepositoryIT` passed, `RoleRepositoryIT` and `UserRepositoryIT` got Connection refused | Workaround (per-machine) | (a) Keep `~/.docker-java.properties` with `api.version=1.44`; permanent fix is upgrading Testcontainers to 1.21.4+. (b) Removed `@Container` annotation; container now starts once via `static { MYSQL.start(); }` in `IntegrationTestBase` and is shared across all IT classes. One MySQL container per JVM run. |
...
- (a) Docker 29.x needs `api.version=1.44` in `~/.docker-java.properties` with Testcontainers 1.20.2 (verified: without it both `EnvironmentAndSystemPropertyClientProviderStrategy` and `NpipeSocketClientProviderStrategy` fail with HTTP 400 BadRequestException).
