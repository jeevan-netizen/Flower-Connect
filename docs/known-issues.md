...
| 017 | Test      | Testcontainers cannot connect to Docker Desktop on Windows (wrong npipe) | Workaround | Integration tests modified to use existing MySQL container at `localhost:3307` instead of Testcontainers-managed container. Original Testcontainers-based `IntegrationTestBase` can be restored for CI/CD environments where Docker works.
...
- Testcontainers (v1.19.8) cannot connect to Docker Desktop on Windows when the default Docker context uses `npipe:////./pipe/docker_cli` while the active `desktop-linux` context uses `npipe:////./pipe/dockerDesktopLinuxEngine`. The `docker` CLI works via Docker contexts, but Testcontainers' `EnvDockerClientProviderStrategy` and `NpipeSocketClientProviderStrategy` read `DOCKER_HOST` env var or system properties. Workaround: temporarily modify `IntegrationTestBase` to point `spring.datasource.url` to the existing MySQL container at `localhost:3307` instead of using Testcontainers. Alternatively, set `DOCKER_HOST` environment variable to `npipe:////./pipe/dockerDesktopLinuxEngine` before running tests.
