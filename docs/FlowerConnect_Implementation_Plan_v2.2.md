# 🌸 FlowerConnect — Implementation Plan (Revision 2.2)
...
### Revision 2.2: Phase 0 and Phase 1 additions

| # | Change | Detail |
|---|---|---|
| 1 | Phase 0 dependencies | Added jjwt, Bucket4j + Caffeine, springdoc-openapi (Swagger UI), spring-boot-starter-mail, Testcontainers MySQL 1.21.4+; lombok-mapstruct-binding annotation-processor order |
| 2 | Frontend tooling | Added ESLint + Prettier, typecheck (tsc --noEmit), @hookform/resolvers; Vite dev proxy for /api |
| 3 | Docker hardening | MySQL healthcheck (depends_on: service_healthy), utf8mb4, named volume, TZ=UTC; prod frontend reverse-proxies /api to backend |
| 4 | Error framework | ErrorCode enum, bean-validation/malformed-JSON handling, AuthenticationEntryPoint + AccessDeniedHandler returning ApiError (401/403) |
| 5 | Test config | application-test.yml externalised; AbstractIntegrationTest with singleton Testcontainers MySQL (static init, no @Container) |
| 6 | Git ignore | 0.10: .gitignore for .env, node_modules, target, uploads |
| 7 | Phase 1 schema | 1.1: users gains full_name; email lowercased; password min 8; phone validated. 1.2: refresh_tokens columns specified; reuse grace window; family revocation |
| 8 | Auth security | 1.4: 409 on duplicate email or phone (D-8); 1.5: reset TTL 30 min, revokes all refresh tokens; 1.6: idempotent bootstrap; 1.9: single-flight guard for StrictMode |
| 9 | Rate limiting | 1.11: forwarded-header IP resolution, hashed email buckets, Caffeine eviction |
| 10 | Phase 1 API | 1.12: PATCH/POST /api/v1/users/me (profile, password); 1.13: scheduled token cleanup |
| 11 | Tests | 1.T: grace window, family revocation, reset revokes sessions, change-password, profile validation, forwarded-header rate limiting, bootstrap idempotency |
...
| **Time** | Inject a `Clock` bean into anything time-dependent (schedulers, expiry, timeouts) so tests can control time. All timestamps stored in UTC (e.g. `DATETIME(6)` in MySQL); use `Instant`/`LocalDateTime` only via the injected `Clock`. |
...
| # | Task | Detail |
|---|---|---|
| 0.1 | Spring Boot init | Web, JPA, Security, Validation, MySQL driver, Lombok, MapStruct, Flyway, Actuator, jjwt, Bucket4j + Caffeine, springdoc-openapi (Swagger UI), spring-boot-starter-mail; Testcontainers MySQL 1.21.4+ added (needed for Docker Engine 29.x); Lombok + MapStruct annotation-processor order (lombok-mapstruct-binding); Mockito for tests |
| 0.2 | React init | Vite, React 18, TypeScript, React Router 6, Axios, TanStack Query, Tailwind, Zustand, react-hook-form, Zod, @hookform/resolvers, ESLint + Prettier, typecheck (tsc --noEmit); Vite dev proxy for /api to backend (same-origin, required by 1.2 cookie design); Vitest + React Testing Library |
| 0.3 | Docker Compose | MySQL 8 with healthcheck (depends_on: condition: service_healthy), utf8mb4, named volume, TZ=UTC; Mailhog, backend, frontend; `frontend/.dockerignore` excluding `node_modules`; service-name URLs inside containers. Prod frontend reverse-proxies /api to backend (single origin). **No Redis in v1** (section 9) |
| 0.4 | Base structure | Package layout and frontend folders from section 5 |
| 0.5 | Flyway baseline | V1 creates `roles` and `users` with ALL columns from 1.1, including `full_name`, `status`, `updated_at`, with unique indexes on email and phone. Timestamps stored in UTC as `DATETIME(6)`. Phase 1 adds V2 for `refresh_tokens` and `password_reset_tokens`. |
...
| 0.7 | Error framework | `ApiError`, `BusinessException` factories, `@RestControllerAdvice`, injected `ObjectMapper`, injectable `Clock` bean; **ErrorCode enum** (VALIDATION_FAILED, UNAUTHORIZED, FORBIDDEN, NOT_FOUND, CONFLICT, RATE_LIMITED, ACCOUNT_SUSPENDED); bean-validation and malformed-JSON error handling; Spring Security `AuthenticationEntryPoint` and `AccessDeniedHandler` returning ApiError (401/403); never leak stack traces |
| 0.8 | Profiles | `application-dev.yml`, `application-prod.yml`, `application-test.yml` skeleton (env-var placeholders), health endpoint; externalised: JWT secret (env only, fails fast in prod if missing, minimum 256-bit), refresh-cookie Secure flag, CORS allowed origin, frontend base URL, server.forward-headers-strategy, refresh-token reuse grace window (seconds) |
| 0.9 | Docs skeleton | `docs/progress.md`, `docs/known-issues.md`, `docs/decisions.md` (records D-1 to D-7); Swagger UI availability documented |
| 0.T | Test baseline | Surefire excludes tag `integration` by default; a Maven profile `integration` runs only tagged tests (Testcontainers, Docker required). Shared `AbstractIntegrationTest` with singleton Testcontainers MySQL, started once in a static initializer, NOT annotated with `@Container` (`@Container` on the shared static field restarts the container per test class). One sample unit test and one sample integration test prove both commands in section 3 work |
| 0.10 | Git ignore | `.gitignore` covering `.env`, `node_modules`, `target`, `uploads` |
...
| # | Task | Detail |
|---|---|---|
| 1.1 | Roles and users | `roles` (seeded lookup: `CUSTOMER`, `VENDOR`, `ADMIN`) and `users`: id, email, **full_name**, phone, password_hash, `role_id` (FK → `roles`; one role per user in v1), status (`ACTIVE`, `SUSPENDED`, `DISABLED`), created_at, updated_at. Email normalised to lowercase. Password policy: minimum 8 characters. Phone format validated. **Vendor approval status lives only in `vendor_profiles`** (single source of truth) |
| 1.2 | Security and tokens | Spring Security + JWT. **Access token:** 15 minutes, returned in the JSON body, held in memory only by the SPA. **Refresh token:** 7 days, in an **httpOnly, Secure, SameSite=Strict cookie scoped to `/api/v1/auth`**, never readable by JavaScript (`Secure` on in prod, configurable off for plain-http local dev). Rotation with reuse detection, BCrypt passwords, refresh tokens stored hashed (SHA-256) and revoked on logout, `@PreAuthorize` by role. Columns: `id`, `user_id`, `token_hash`, `family_id`, `expires_at`, `revoked_at`, `replaced_by_id`, `created_at`. Reuse of a rotated token revokes the whole family; a short grace window (e.g. 10 seconds, configurable per 0.8) tolerates concurrent refreshes from multiple tabs. Security 401/403 responses use the handlers from 0.7. Finalises D-5. SPA and API must be same-site in every environment (Vite dev proxy locally, one reverse-proxied origin in prod); CORS allows credentials for the exact configured origin only |
...
| 1.4 | Auth API | `POST /api/v1/auth/register` (**customer only**; returns **409** with a specific message on a duplicate email or phone — UX trade-off, see D-8), `/login`, `/refresh` (reads the cookie, returns a new access token, rotates the cookie), `/logout` (revokes and clears the cookie), `GET /api/v1/users/me`. "No user enumeration" acceptance criterion (section 14) is scoped to **login and forgot-password** only |
| 1.5 | Password reset | `/auth/forgot-password`, `/auth/reset-password`; reset token TTL 30 minutes; single-use, expiring, hashed token; email via Mailhog; identical response whether or not the email exists; throttled by 1.11. A successful reset revokes all the user's refresh tokens. The email link uses the configurable frontend base URL (0.8) |
| 1.6 | Admin bootstrap | Bootstrap is idempotent and skipped if any ADMIN exists. Requires env credentials; no default password outside the dev profile. First admin created from env-configured credentials on first boot (dev seed); no public admin registration |
...
| 1.9 | Auth store and interceptor | Zustand store holds the access token **in memory only**: no `localStorage` / `sessionStorage` persistence of tokens. Axios uses `withCredentials`. On app load a silent `/auth/refresh` restores the session; the "initial load finished" flag is set on success **and** failure. The silent refresh must be guarded by a module-level single-flight promise so React StrictMode's double-invoke in dev does not trigger reuse detection. On 401 the interceptor refreshes once (single-flight) and retries, and **skips public endpoints** (a 401 from `/auth/refresh` must not trigger another refresh). `/auth/refresh` and `/auth/logout` are cookie-authenticated, so they require a custom header (e.g. `X-Requested-With`) and an `Origin` check; every other endpoint uses the Bearer header and is not exposed to CSRF |
...
| 1.11 | Rate limiting | Bucket4j (in memory) from day one: login and register per IP and per email, refresh per IP, forgot/reset password limited per email and per IP (e.g. 5 requests per hour); 429 with `Retry-After`. Limits live in configuration. Client IP resolved via forwarded headers (per 0.8), trusted only when set by the frontend reverse proxy. Buckets keyed by hashed email. Caffeine-backed eviction so buckets do not grow unbounded. |
| 1.12 | Profile and password | `PATCH /api/v1/users/me` (full_name, phone); `POST /api/v1/users/me/password` (change password; requires current password; revokes the user's other refresh tokens) |
| 1.13 | Token cleanup | Scheduled cleanup of expired/revoked refresh and reset tokens, using the injected Clock |
| 1.T | Tests | Register/login/refresh/logout flows; rotation reuse detection; reuse grace window; family revocation; reset-token expiry and single use; password reset revoking sessions; change-password; profile update validation; **status matrix** (suspended and disabled users cannot log in, refresh, or use an already-issued access token); cookie flags; refresh without the custom header rejected; 429 behaviour; rate-limit keys behind a forwarded header; admin bootstrap idempotent |
...
| Phase | Hours |
|---|---|
| 0 Scaffolding and foundations | 6–8 |
| 1 Auth | 10–12 |
...
| **Total (coding effort)** | **95–120** |

- These are rough estimates for hands-on build time. With per-diff review and real verification on every phase, plan for roughly **1.5×: about 143–180 hours**.
...
- [ ] Auth endpoints are rate-limited; uploads are validated; no stack traces; no user enumeration in login and forgot-password responses
